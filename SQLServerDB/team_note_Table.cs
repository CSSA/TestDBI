...
{
    public class team_note_Table
    {
        public const string theTable = "T_team_note";

        public List<team_note> itemList = new List<team_note>();
        ...
        ///</summary>
        ///<input>List<team_note> itemList - an ordinary List<> of type team_note, will be cleared if not already empty </input>
        ///<output>List<team_note>  itemList - an ordinary List<> of type team_note, extracted from the database </output>
        public void ReadItemListFromDatabase()
        {
            ...
            if (dsObj != null && dsObj.Tables[0].Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dtObj.Rows)
                {
                    ...
                    //fill the itemList
                    team_note newRec = new team_note();
                    newRec.ID = intID;
        ...
        ///<output>List<team_note> itemList - an ordinary List<> of type team_note, output to the "theTable" in the database </output>
        ...
        ///</summary>
        ///<output>team_note r - output one team_note object to the "theTable" in the database </output>
        public void WriteItemToDatabase(team_note r)
        {
            SqlConnection myConnection = DBUtils.GetNewSqlConnection();
            if (myConnection == null)
            {
                LogManager.writeToLog("New connection failed to open; team_note_Table.cs:WriteItemToDatabase.GetNewSqlConnection()");
                return;
            }
        ...
        ///</summary>
        ///<input>List<team_note> itemList - an ordinary List<T> of type team_note, updated within the "theTable" in the database </input>
        public void UpdateItemListToDatabase()
        {
            SqlConnection myConnection = DBUtils.GetNewSqlConnection();
            if (myConnection == null)
            {
                LogManager.writeToLog("New connection failed to open; team_note_Table:UpdateItemListToDatabase");
                return;
            }
        ...
        ///</summary>
        ///<input>team_note r -  one item to be updated within the "theTable" in the database </input>
        ///<input> r.currentProject - the projectId to match with one database record</input>
        public void UpdateItemToDatabase(team_note r)
        {
            SqlConnection myConnection = DBUtils.GetNewSqlConnection();
            if (myConnection == null)
            {
                LogManager.writeToLog("New connection failed to open; in team_note_Table.cs:UpdateItemToDatabase.GetNewSqlConnection()");
                return;
            }
        ...
        //---------------------------------------------------------------------------------------------------------------
        /// <summary>
        /// Clear_Database_Table - delete all records from the table "team_note"  database
        ...
        public void Clear_Database_Table()
        {
            ...
            if (!DBUtils.ExecuteSqlNonQuery(strCommand))
                LogManager.writeToLog(" ExecuteSqlNonQuery returned: FALSE; in team_note_Table.cs:Clear_Database_Table.ExecuteSqlNonQuery()");
        }
        ...
        public int CountRows_By_projectId(int projectId)
        {
            ...
            if (myConnection == null)
            {
                LogManager.writeToLog("New connection failed to open; team_note_Table.cs:SQLServer_CountRows_By_projectId.GetNewSqlConnection");
                return -1;
            }
            string strQuery = "SELECT COUNT(*)  FROM " + theTable +
             "  WHERE " + "projectId=" + projectId.ToString();
