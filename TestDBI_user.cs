...
namespace TestDBI
{
    class TestDBI_user
    {
        ...
        static void TestDBI_T_user_AutoCheck()
        {
            ...
            iResult = TestDBI_T_user_AutoCheck_WriteRead();
            if (iResult == 0)
            ...
            iResult = TestDBI_T_user_AutoCheck_Update();
            if (iResult == 0)
                Console.WriteLine("OK: TestDBI_T_user_AutoCheck_Update");
            else
                Console.WriteLine("ERROR: TestDBI_T_user_AutoCheck_Update:    iResult=" + iResult);
        ...
        /// <summary>
        /// TestDBI_T_user_AutoCheck_Update - Update Item List;
        /// 1.1) Create test data: myTable1;
        /// 1.2) Clear DBTable;
        /// 1.3) Write myTable1 to DBTable; 
        /// 1.4) Get DBTable.CountRows, compare (myTable1.itemList.Count == DBTable.CountRows)
        ...
        /// </summary>
        /// <returns></returns>
        static int TestDBI_T_user_AutoCheck_Update()
        {
            const int OK = 0;
            int iResult = OK;
            Console.WriteLine("START: TestDBI_T_user_AutoCheck_Update()");
           ...
           new user("username_5","org_1", "password_5"),
            ...
            /// 1.7) Create the update table (myTableUpdate)
            user_Table myTableUpdate = new user_Table();
            myTableUpdate.itemList = new List<user>()
            {
                // user(String val_username, String val_organization, String val_password)
           new user("username_1","org_1", "password_1"),
           new user("username_2","org_1", "password_2"),
           ...
           new user("username_5","org_1", "password_5"),
            ...
            //1.9) Read myTable3
            user_Table myTable3 = new user_Table();
            myTable3.ReadItemListFromDatabase();


            //1.10) Compare tables.itemLists (myTableUpdate == myTable3)
            if (!TestDBI_T_user_CompareLists(myTableUpdate.itemList, myTable3.itemList))
            {
                iResult = -1;
            ...
            Console.WriteLine("DONE: TestDBI_T_user_AutoCheck_Update()");
            return iResult;
        }
